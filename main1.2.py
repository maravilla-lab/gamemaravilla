...
class MaravillaGame(QWidget):
    ...
    def __init__(self):
        ...
        # Señales de tus archivos originales
    ...
    def render_biblioteca(self):
        ...
        for cat in cats:
            ...
            for i, item in enumerate([x for x in self.trivias if x['cat'] == cat]):
                ...
                # Estilo solicitado: fondo verde si ya se usó
    ...
    def actualizar_ui(self): self.lbl_stats.setText(f"💎 {self.monedas} | XP: {self.puntos} | Nivel: {self.dificultad_actual}")
    ...
    def enviar_chat(self):
        if self.chat_in.text(): self.sio.emit('enviar_mensaje', {'user': self.uid, 'msg': self.chat_in.text()}); self.chat_in.clear()
