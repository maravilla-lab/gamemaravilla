...
class MaravillaGame(QWidget):
    ...
    def __init__(self):
        ...
        def on_stats(d): self.signal_resultado.emit(d)
    ...
    def init_ui(self):
        ...
        # Ranking Superior
        self.rank_box = QLabel("🏆 CARGANDO LÍDER..."); self.rank_box.setFixedHeight(65); self.rank_box.setAlignment(Qt.AlignCenter)
        self.rank_box.setStyleSheet("background:#0a0a0a; color:#ffee00; border:2px solid #ffee00; border-radius:10px; font-weight:bold; font-size:11pt; padding:5px;")
        ...
        # Header de Stats
        header = QFrame(); header.setFixedHeight(55); header.setStyleSheet("background:#111; border-radius:10px; border:1px solid #00ffcc;")
        h_lay = QHBoxLayout(header)
        self.lbl_stats = QLabel(); self.lbl_stats.setStyleSheet("font-size:15px; font-weight:bold; color:#00ffcc;"); h_lay.addWidget(self.lbl_stats)
        
        self.btn_mute = QPushButton("🔊"); self.btn_mute.setFixedSize(40, 35); self.btn_mute.clicked.connect(self.toggle_mute)
        h_lay.addWidget(self.btn_mute)

        btn_pay = QPushButton("💲 RECARGAR"); btn_pay.setStyleSheet("background:#ff0050; color:white; font-weight:bold; border-radius:5px; height:35px; width:100px;")
        ...
        # Botones de Colores Grandes
        grid = QGridLayout()
        self.btns = {}
        for i, (n, c) in enumerate([("Rojo","#ff0050"), ("Azul","#00f2ea"), ("Verde","#00ff88"), ("Amarillo","#ffee00")]):
            b = QPushButton(n); b.setFixedSize(125, 125); b.setEnabled(False)
            b.setStyleSheet(f"background:{c}; color:black; border-radius:62px; border:4px solid #000; font-weight:bold; font-size:16px;")
        ...
        self.tabs = QTabWidget(); self.tabs.setStyleSheet("QTabBar::tab { background: #111; color: white; padding: 10px; } QTabBar::tab:selected { border-bottom: 2px solid #00ffcc; }")
        lay.addWidget(self.tabs)

        # Chat
        self.chat_view = QTextEdit(); self.chat_view.setReadOnly(True); self.chat_view.setFixedHeight(110)
        self.chat_in = QLineEdit(); self.chat_in.setPlaceholderText("Mensaje al servidor..."); self.chat_in.returnPressed.connect(self.enviar_chat)
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in teclas:
            c = teclas[event.key()]
            if self.btns[c].isEnabled(): self.clic_color(c)
    ...
    def procesar_resultado(self, d):
        if 'stats' in d:
            self.puntos = d['stats']['puntos']
            self.monedas = d['stats']['monedas']
    ...
    def render_biblioteca(self):
        ...
        for cat in cats:
            ...
            for i, it in enumerate([x for x in self.trivias if x['cat'] == cat]):
                ...
                if not ya: btn.clicked.connect(lambda _, x=it: self.pedir_trivia(x))
    ...
    def pedir_trivia(self, item):
        if self.monedas >= item['costo']:
            ...
            res, ok = QInputDialog.getText(self, "Misión Maravilla", item['preg'])
    ...
    def ejecutar_evento_especial(self, d):
        if d['tipo'] == 'regalo': self.chat_view.append(f"<b style='color:gold;'>🎁 {d['user']} {d['msg']}</b>")
