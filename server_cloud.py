...
# --- CONFIGURACIÓN DE ECONOMÍA (Basado en tus archivos) ---
...
TRIVIAS_MAESTRAS = [
    ...
    {"id": 300, "cat": " Socios 🤝", "tit": "Socio Bronce", "costo": 500, "url": "https://www.tiktok.com/@portal.maravilla", "preg": "Código Socio:", "res": "socio1", "premio": 1500, "xp": 2000},
...
def login():
    ...
    return jsonify({"stats": usuarios[uid], "trivias": trivias_pub, "ranking": obtener_ranking(), "online": len(usuarios)})
...
def verificar(data):
    ...
    if trivia and res == trivia['res'] and u in usuarios:
        if t_id not in usuarios[u].get('logros', []):
            usuarios[u]['monedas'] += trivia.get('premio', 500)
